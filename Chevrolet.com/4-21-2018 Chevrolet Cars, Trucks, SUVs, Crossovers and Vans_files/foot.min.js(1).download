...
/*!
 * @preserve
 * @date      Mon Apr 16 2018 18:35:59 GMT-0400 (EDT)
 * @time      1523918166620
 * @version   1.27.0.0-SNAPSHOT-123
...
}},buildEndPoint:function(f){var e=a.find(a.get(this.endPoints,"endPointList"),a.matchesProperty("endPointTag",f)),c="",d=this.$checkboxSelector.is(":checked");
if(e&&a.hasIn(e,"endPointUrl")&&e.endPointUrl){c=e.endPointUrl;
...
}},processDealers:function(h){var g=h&&h.status&&h.status==="success"&&a.hasIn(h,"payload.dealers")&&h.payload.dealers?h.payload.dealers:[],f={certification:[]},c={},e={valid:true},d=this.$checkboxSelector.is(":checked");
...
this.$eCommError=this.$requestType.find(this.cfg.eCommError);
...
}},finish:function(){a("#gmit-gf-submit-request-type").on("click",a.proxy(this.ecomButtonValidator,this))
...
}gmwp.util.loadScript(b.epgScript).then(function(){b.widget=new GMPaymentUI({frame:"#widget",debug:b.widgetDebug,merchantId:b.merchantId,clearErrorOnChange:true,clearErrorOnFocus:false,clearErrorOnBlur:false,showCardImages:true,disableJavaScriptClickjackMitigation:true,formFields:{nameOnCard:{label:b.nameOnCardLabel,placeholder:"",requiredError:b.requiredErrorBase+" ("+b.nameOnCardLabel+")",invalidError:b.invalidErrorBase+" "+b.nameOnCardLabel},creditCard:{label:b.creditCardLabel,placeholder:b.creditCardPlaceholder,requiredError:b.requiredErrorBase+" ("+b.creditCardLabel+")",invalidError:b.invalidErrorBase+" "+b.creditCardLabel},expiryMonth:{label:b.expiryMonthLabel,placeholder:b.expiryMonthPlaceholder,requiredError:b.requiredErrorBase+" ("+b.expiryMonthLabel+")",invalidError:b.invalidErrorBase+" "+b.expiryMonthLabel},expiryYear:{label:b.expiryYearLabel,placeholder:b.expiryYearPlaceholder,requiredError:b.requiredErrorBase+" ("+b.expiryYearLabel+")",invalidError:b.invalidErrorBase+" "+b.expiryYearLabel},securityCode:{label:b.securityCodeLabel,placeholder:"",requiredError:b.requiredErrorBase+" ("+b.securityCodeLabel+")",invalidError:b.invalidErrorBase+" "+b.securityCodeLabel}},onSuccessCallback:a.proxy(b.widgetSuccess,b),onErrorCallback:a.proxy(b.widgetError,b)})
})
},ecomButtonValidator:function(c){var b=this.$ecomCheckboxSelector.is(":checked");
c.preventDefault();
if(b){if(this.validateElements(this.$arrOfFields)){this.widget.submit()
}}else{this.$form.submit()
}},validateElement:function(d){var e=d.parent(),b=e.find("ul"),c=d.data("validations");
if(d.is('input[type="radio"]')||d.is('input[type="checkbox"]')){e=d.closest(".gmit-gf-field-options");
b=e.find(".gmit-gf-field-group");
c=e.find(".gmit-gf-field-group").data("validations")
}return gmwp.util.gf.applyValidations(c,d,e,b)
},validateElements:function(b){var e=true,c;
for(var d=0;
d<b.length;
d++){if(!this.validateElement(b[d])){e=false
}}c=gmwp.$(".gmit-gf-input-radio").filter(":checked")[1].id;
if(c==="preferred-comm-Home_Email"){if(!this.validateElement(this.$emailAdress)){e=false
}}else{if(!this.validateElement(this.$phoneNumber)){e=false
}}return e
},widgetSuccess:function(h){var l,g,i,j=h.customer,c=new XMLHttpRequest(),k=a("#state").val(),e=a("#address_1").val()+" "+a("#address_2").val(),m=a("#gmit-gf-postal-code").val(),f=a("#cityname").val(),d={},b={};
a("#gmit-gf-hidden-comments").append(" Session ID: "+h.sessionId);
l={sessionId:h.sessionId,merchantId:this.merchantId,nameOnCard:h.payment.nameOnCard,device:{ipAddress:j&&j.ipAddress?j.ipAddress:null},order:{currency:this.currency,amount:this.amount,owningEntity:this.owningEntity},billing:{address:{stateProvince:k,street:e,postcodeZip:m,country:"USA",city:f}}};
d={APPROVED:3,UNSPECIFIED_FAILURE:2,DECLINED:2,TIMED_OUT:1,EXPIRED_CARD:2,INSUFFICIENT_FUNDS:2,ACQUIRER_SYSTEM_ERROR:2,SYSTEM_ERROR:1,NOT_SUPPORTED:1,DECLINED_DO_NOT_CONTACT:2,ABORTED:3,BLOCKED:2,CANCELLED:3,DEFERRED_TRANSACTION_RECEIVED:3,REFERRED:2,AUTHENTICATION_FAILED:3,INVALID_CSC:2,LOCK_FAILURE:1,SUBMITTED:3,NOT_ENROLLED_3D_SECURE:3,PENDING:3,EXCEEDED_RETRY_LIMIT:1,DUPLICATE_BATCH:1,DECLINED_AVS:2,DECLINED_CSC:2,DECLINED_AVS_CSC:2,DECLINED_PAYMENT_PLAN:2,APPROVED_PENDING_SETTLEMENT:3,PARTIALLY_APPROVED:2,UNKNOWN:2};
b={1:this.technicalErrorMessage,2:this.paymentErrorMessage,3:"n/a"};
if(this.flagSubmitPay===false){c.open("POST",this.epgEndpoint,false);
c.setRequestHeader("Content-Type","application/json");
c.send(JSON.stringify(l));
gmwp.util.log(c.responseText);
g=JSON.parse(c.responseText);
gmwp.util.log(g);
if(g.result==="SUCCESS"){gmwp.util.log("Payment Successful");
this.flagSubmitPay=true;
this.$eCommError.html("");
this.$form.submit()
}else{gmwp.util.log("Error with "+g.result);
if(g.result==="ERROR"){switch(g.error.cause){case"INVALID_REQUEST":this.$eCommError.html(this.paymentErrorMessage);
break;
default:this.$eCommError.html(this.technicalErrorMessage)
}}else{if(g.result==="FAILURE"){i=g.response.gatewayCode;
this.$eCommError.html(b[d[i]])
}else{this.$eCommError.html(this.technicalErrorMessage)
}}}}else{gmwp.util.log("Payment Already Succesful, Resubmitting Form");
this.$form.submit()
}},widgetError:function(b){gmwp.util.error("gmit_gf_ecommerce::widgetError",b);
...
(function(b,a){b.extend(true,gmwp.conf.cmp,{gmit_gf_map:{name:"gmit_gf_map",dataApiKey:"api-key",dataGoogleApiUrl:"google-api-url",initMapFunction:"window.gmitGfInitMap",latDrilldown:"coordinates.latitude",lngDrilldown:"coordinates.longitude",selector:".gmit-gf-map",listSelector:".gmit-gf-list",markerImgSrc:"https://web.archive.org/web/20180420042158/http://www.google.com/mapfiles/marker{label}.png"}});
...
/*
     FILE ARCHIVED ON 04:21:58 Apr 20, 2018 AND RETRIEVED FROM THE
...
/*
playback timings (ms):
  captures_list: 595.205
  exclusion.robots: 0.11
  exclusion.robots.policy: 0.097
  RedisCDXSource: 0.653
  esindex: 0.009
  LoadShardBlock: 567.459 (3)
  PetaboxLoader3.datanode: 77.448 (4)
  PetaboxLoader3.resolve: 1153.242 (2)
  load_resource: 689.637
*/