...
/*!
 * @preserve
 * @date      Fri Jun 01 2018 19:26:15 GMT-0400 (EDT)
 * @time      1527895582297
 * @version   1.28.0.3-SNAPSHOT-217
...
},setupOptionList:function(g,e){var d="",j="code",f="",k=e||"modelyear",i="",h="title",c=b("<option></option>");
if(g){d=a.has(g,j)?g[j]:"";
i=a.hasIn(g,h)?g[h]:"";
c.attr("value",d);
c.text(i);
if(k){f=k+"."+d+"<sep/>";
c.attr("data-feature-id",f)
}}return c
...
i.html("");
a.each(e,b.proxy(function(u){if(a.isPlainObject(u)){if(u.effect==="error_message"){switch(u.operator){case"cpf":if(!this.cpfValidation(p)){i.append(this.addErrorLabel(u.message));
m=false
}break;
case"mandatory":if(p.is("select")){j=p.find("option:selected");
...
}}},cpfValidation:function(g){var c,d,f,e;
if(g===undefined||!g){return true
}d=g.val();
if(!d){return true
}if(!d.match(/^(\d{11}|\d{3}[\.]?\d{3}[\.]?\d{3}[\-]?\d{2})$/)){return false
}d=d.replace(/\.|\-/g,"");
if(d.length!==11){return false
}if(d==="00000000000"||d==="11111111111"||d==="22222222222"||d==="33333333333"||d==="44444444444"||d==="55555555555"||d==="66666666666"||d==="77777777777"||d==="88888888888"||d==="99999999999"){return false
}c=d.split("");
c.forEach(function(i,h,j){j[h]=parseInt(i,10)
});
f=10*c[0]+9*c[1]+8*c[2]+7*c[3]+6*c[4]+5*c[5]+4*c[6]+3*c[7]+2*c[8];
f=11-f%11;
if(f>=10){f=0
}e=11*c[0]+10*c[1]+9*c[2]+8*c[3]+7*c[4]+6*c[5]+5*c[6]+4*c[7]+3*c[8];
e+=2*f;
e=11-e%11;
if(e>=10){e=0
}if(f!==c[9]||e!==c[10]){return false
}return true
...
this.sandbox.subscribe(this.id,this);
this.subscribe("gmit_gf_ecommerce",this)
...
gmwp.mod.gmit_gf_checkbox_field=gmwp.mod.AtomicMod.extend({events:{click:[{}]},initElements:function(){var b=this.$ctx;
...
this.$dealerAddress1=c.find(this.cfg.dealerAddress1);
this.$dealerAddress2=c.find(this.cfg.dealerAddress2);
...
}if(h.address.addressLine2){this.$dealerAddress2.html(h.address.addressLine2)
...
this.$validationElements=c.find(this.cfg.validationItems);
this.$radioButtons=this.$searchOption.find(this.cfg.radioButtonSelector);
this.$checkboxSelector=b(this.cfg.checkboxSelector);
this.$searchInput=this.$searchText.find(this.cfg.searchInputSelector);
...
this.endPoints=c.data(this.cfg.dataEndPoints);
this.$selectedDealerAddress=this.$ctx.find(this.cfg.selectedDealerAddress);
this.$selectedDealerName=this.$ctx.find(this.cfg.selectedDealerName);
this.dealer="";
this.dealerObj={};
...
this.$ecommerceWidget=b(this.cfg.ecommerceWidget);
this.ecommerceWidgetId=gmwp.util.getModuleId(this.$ecommerceWidget);
...
this.checkboxBfu=c.data(this.cfg.dataCheckboxBfu);
this.zipValidation=c.data(this.cfg.dataZipValidation);
this.searchUrl=c.data(this.cfg.dataSearchUrl);
this.sandbox.subscribe(this.id,this)
...
}if(this.ecommerceWidgetId>-1){this.sandbox.subscribe(this.ecommerceWidgetId,this)
}this.subscribe(gmwp.conf.cmp.gmit_gf_checkbox_field.name,this)
...
break;
default:break
}b(this.cfg.checkboxSelector).on("click",b.proxy(this.resetSelections,this));
this.loadPreSelectedDealer()
...
}},buildEndPoint:function(h){var e=a.find(a.get(this.endPoints,"endPointList"),a.matchesProperty("endPointTag",h)),c="",d=this.$checkboxSelector.is(":checked"),f,g;
if(e&&a.hasIn(e,"endPointUrl")&&e.endPointUrl){f=e.endPointUrl;
if(!a.isEmpty(this.searchUrl)&&/\/dealer\/\w+\/v1\//.test(f)){if(a.startsWith(f,"http")){g=f.match(/^(http[s]?:\/)?\/?([^:\/\s]+\/)/);
c=g[0].substr(0,g[0].length-1)
}if(a.endsWith(this.searchUrl,"/")){this.searchUrl=this.searchUrl.substring(0,this.searchUrl.length-1)
}else{c+=this.searchUrl
}g=f.match(/(\/\w+\/v1\/)/);
c+=f.substr(f.indexOf(g[0]))
}else{c=e.endPointUrl
}if(a.hasIn(e,"urlParameters")&&e.urlParameters){c=c+"?"+b.param(e.urlParameters)
}}if(this.checkboxBfu&&d){c=c+"&services="+this.checkboxBfu+"%7C"
}return c
...
if(d){c=d.addressLine1?d.addressLine1+", ":"";
c+=d.addressLine2?d.addressLine2+", ":"";
...
}}},dealerSearchAjaxCall:function(c){if((a.includes(c,"/dealer/cityState")||a.includes(c,"/dealer/postalCode")&&/^\d{5}\-?\d{3}$/.test(this.searchValue))&&this.locale==="pt_BR"){this.mapLinkSearch(c)
}else{this.ocDealerSearch(c)
}},ocDealerSearch:function(c){b.ajax({url:c,dataType:"json",contentType:"application/x-www-form-urlencoded; charset=UTF-8",method:"GET",statusCode:{404:b.proxy(this.showNotFoundError,this),400:b.proxy(this.showNotFoundError,this),500:b.proxy(this.showNotFoundError,this)},beforeSend:b.proxy(this.setHeaders,this),success:b.proxy(this.processDealers,this),fail:b.proxy(this.showNotFoundError,this),complete:b.proxy(this.processComplete,this)})
},mapLinkSearch:function(g){var e="",d,f,c,j,i,h;
c=this;
d=(this.$checkedRadio.val()||"").toLocaleLowerCase();
if(a.startsWith(g,"http")){h=g.match(/^(http[s]?:\/)?\/?([^:\/\s]+\/)/);
e=h[0].substr(0,h[0].length-1)
}e+="/bypass/maplink/v0/geocode/geocode?token=zCB8zCB8OXclPU==";
if(d==="zip"){jQuery.get(e,{postalcode:this.searchValue}).then(function(k){c.mapLinkSuccess(k,d)
}).fail(function(){c.mapLinkFail(g)
})
}else{if(d==="city"){f=this.searchValue.split(",");
j=f[0].trim().replace(/\s+/g,"+");
i=f[1].trim();
jQuery.get(e,{state:i,city:j}).then(function(k){c.mapLinkSuccess(k,d)
}).fail(function(){c.mapLinkFail(g)
})
}}},mapLinkSuccess:function(d,f){var i,e,g,h,c;
if(a.isEmpty(d.addresses)){return
}c=d.addresses[0].street||d.addresses[0].district||d.addresses[0].city;
i=c.center.coordinates;
g=a.find(this.endPoints.endPointList,a.matchesProperty("endPointTag",f));
e=g.endPointUrl;
if(f==="zip"){e=e.replace("postalCode","latlong")
}else{if(f==="city"){e=e.replace("cityState","latlong")
}}h=g.urlParameters;
e=e.replace("{searchValue}",i.reverse().join("/"));
e=h.reduce(function(j,k){j+=k.name+"="+encodeURIComponent(k.value)+"&";
return j
},e+"?");
this.ocDealerSearch(e)
},mapLinkFail:function(c){this.ocDealerSearch(c)
...
}},processDealers:function(h){var g=h&&h.status&&h.status==="success"&&a.hasIn(h,"payload.dealers")&&h.payload.dealers?gmwp.util.formatDealerUrls(h.payload.dealers):[],f={certification:[]},c={},e={valid:true},d=this.$checkboxSelector.is(":checked");
a.set(c,"exclude.activity",[]);
a.each(this.filterSeviceTypes,function(i){f.certification.push(i)
});
a.each(this.excludeActivities,function(i){c.exclude.activity.push(i)
});
if(f.certification.length){g=a.filter(g,f)
}if(c.exclude.activity.length){a.remove(g,c)
}if(g.length){this.setDealerResultHeadline(g);
a.each(g,b.proxy(this.addDealer,this))
}else{this.$searchOptionContainer.show();
this.$closeDealer.show();
this.$newSearchSelector.show();
...
}if(this.checkboxBfu&&d){this.$checkboxSelector.trigger("click");
this.showEcommError();
this.$searchButton.click()
}else{this.showNotFoundError(h,true)
}}if(g.length===1&&this.searchResultComponentCheck){this.fire("gmitGfSelectFirstItem",[this.searchResultComponentId]);
this.fire("gmitGfDealerSelected",e,[this.id]);
if(this.dealerEditable!=="true"){this.$closeDealer.hide();
this.$newSearchSelector.hide();
if(this.searchResultComponentCheck){this.fire("gmitGfDisableList",[this.searchResultComponentId])
}}}if(g.length>1&&this.searchResultComponentCheck){this.fire("gmitGfShowSearchResult",[this.searchResultComponentId])
...
},onGmitGfItemSelected:function(f){var e=this.dealer||{},g=[],h,c={},d=this.$checkboxSelector.is(":checked");
this.dealer=f;
this.dealerObj=this.dealer||{};
this.$selectedDealerAddress.empty();
this.$selectedDealerName.empty();
this.$selectedDealer.data(this.cfg.dataDealer,this.dealer);
if(this.dealer){if(a.hasIn(this.dealer,"cadc")&&this.dealer.cadc){h=this.dealer.cadc
}else{if(a.hasIn(this.dealer,"id")&&this.dealer.id){h=this.dealer.id.toString()
}}this.dealerObj.id=h;
this.dealerObj.dealerName=this.dealerObj.dealerName||"";
this.setDefaultValueIfUndefined(this.dealerObj,"address.addressLine1","");
this.setDefaultValueIfUndefined(this.dealerObj,"address.addressLine2","");
this.setDefaultValueIfUndefined(this.dealerObj,"address.cityName","");
this.setDefaultValueIfUndefined(this.dealerObj,"address.postalCode","");
this.setDefaultValueIfUndefined(this.dealerObj,"address.countryCode","");
this.dealerObj.distance=this.dealerObj.distance||"";
this.setDefaultValueIfUndefined(this.dealerObj,"address.countrySubdivisionCode","");
this.dealerObj.id=this.dealerObj.id||"";
this.setDefaultValueIfUndefined(this.dealerObj,"generalContact.phone1","");
this.dealerObj.generalOpeningHour=this.dealerObj.generalOpeningHour||"";
this.dealerObj.dealerUrl=this.dealerObj.dealerUrl||"";
this.setDefaultValueIfUndefined(this.dealerObj,"address.countryIso","");
if(a.hasIn(this.dealer,"selected")&&this.dealer.selected){e.valid=true;
if(a.hasIn(this.dealer,"bac")&&this.dealer.bac){this.$hiddenValues.find(this.cfg.dealerBac).val(this.dealer.bac);
if(this.checkboxBfu&&d){this.fire("gmitGfEcomDealerSelected",this.dealer,[this.id])
}}if(a.hasIn(this.dealer,"upsc")&&this.dealer.upsc){this.$hiddenValues.find(this.cfg.dealerUpsc).val(this.dealer.upsc)
}if(this.$hiddenValues.find(this.cfg.dealerId).val()){g=a.split(this.$hiddenValues.find(this.cfg.dealerId).val(),",")
}if(!a.includes(g,h)){g.push(h);
this.$hiddenValues.find(this.cfg.dealerId).val(g.toString());
if(a.hasIn(this.dealer,"geolocation")&&this.dealer.geolocation){c.name=h;
...
this.$hiddenValues.find(this.cfg.dealerName).val(this.dealer.dealerName)
}if(a.hasIn(this.dealer,"address")&&this.dealer.address){this.$selectedDealerAddress.html(this.formatAddress(this.dealer.address));
...
}if(a.hasIn(this.dealer.address,"addressLine2")&&this.dealer.address.addressLine2){this.$hiddenValues.find(this.cfg.dealerStreet2).val(this.dealer.address.addressLine2)
...
}else{if(this.$hiddenValues.find(this.cfg.dealerId).val()){g=a.split(this.$hiddenValues.find(this.cfg.dealerId).val(),",")
}if(a.includes(g,h)){a.pull(g,h);
this.$hiddenValues.find(this.cfg.dealerId).val(g.toString());
if(a.hasIn(this.dealer,"geolocation")&&this.dealer.geolocation){c.name=h;
c.coordinates=this.dealer.geolocation;
a.pullAllWith(this.markersObj.markers,[c],a.isEqual)
}if(a.isEmpty(g)){e.valid=false;
this.fire("gmitGfEcomDealerSelected",null,[this.id]);
...
this.fire("gmitGfDealerSelected",e,[this.id])
},setDefaultValueIfUndefined:function(d,e,c){if(!a.hasIn(d,e)){a.set(d,e,c)
...
}if(this.$searchInput.filter("[data-invalid]").length){if(this.showDealerErrorVisible){this.onResetshowDealerErrorVisible();
Foundation.libs.abide.validate(this.$searchInput,{type:""})
...
}},resetSelections:function(){this.fire("gmitGfEcomDealerSelected",null,[this.id]);
this.fire("gmitGfClearSearchResult",[this.searchResultComponentId])
},showEcommError:function(){var c="No eCommerce dealers found for selected search";
this.$searchOptionContainer.show();
if(this.searchResultComponentCheck){this.fire("gmitGfShowCriteriaError",c,[this.searchResultComponentId])
}}})
})(gmwp.$,gmwp._);
...
(function(a){a.extend(true,gmwp.conf.cmp,{gmit_gf_ecommerce:{name:"gmit_gf_ecommerce",address:"#address_1",cityName:"#cityname",ecomCheckboxSelector:"#gmit-gf-ecommerce-checkbox",eCommError:".ecommerce-error",emailAddress:"#email_address",firstName:"#gmit-gf-first-name",formSelector:"form",hideClass:"gmit-gf-hide",lastName:"#gmit-gf-last-name",phoneNumber:"#telephone_phone_number",stateName:"#state",zipCodeAddress:"#gmit-gf-postal-code",zipCodeDealer:"#gmit-gf-search-text",dataEpgEndpoint:"epg-endpoint",dataEpgScript:"epg-script",dataCurrency:"currency",dataAmount:"amount",dataOwningEntity:"owning-entity",dataWidgetDebug:"widget-debug",dataTestMerchantId:"test-merchant-id",dataPrefixMerchantId:"prefix-merchant-id",dataTechnicalErrorMessage:"technical-error-message",dataPaymentErrorMessage:"payment-error-message",dataNameOnCardLabel:"name-on-card-label",dataCreditCardLabel:"credit-card-label",dataExpiryMonthLabel:"expiry-month-label",dataExpiryYearLabel:"expiry-year-label",dataSecurityCodeLabel:"security-code-label",dataCreditCardPlaceholder:"credit-card-placeholder",dataExpiryMonthPlaceholder:"expiry-month-placeholder",dataExpiryYearPlaceholder:"expiry-year-placeholder",dataInvalidErrorBase:"invalid-error-base",dataRequiredErrorBase:"required-error-base",dealerSelectorV2:".gmit-gf-dealer-selector-v2",requestType:".gmit-gf-request-type"}});
gmwp.mod.gmit_gf_ecommerce=gmwp.mod.AtomicMod.extend({initElements:function(){this.widget=null;
this.merchantId=null;
this.flagSubmitPay=false;
this.deferredObj=null;
this.$form=a(this.cfg.formSelector);
this.$zipCodeDealer=a(this.cfg.zipCodeDealer);
this.$firstName=a(this.cfg.firstName);
this.$lastName=a(this.cfg.lastName);
this.$address=a(this.cfg.address);
this.$cityName=a(this.cfg.cityName);
this.$stateName=a(this.cfg.stateName);
this.$zipCodeAddress=a(this.cfg.zipCodeAddress);
this.$emailAdress=a(this.cfg.emailAddress);
this.$phoneNumber=a(this.cfg.phoneNumber);
this.$payName=a(this.cfg.payName);
this.$payMonth=a(this.cfg.payMonth);
this.$payYear=a(this.cfg.payYear);
this.$arrOfFields=[this.$zipCodeDealer,this.$firstName,this.$lastName,this.$address,this.$cityName,this.$stateName];
this.$requestType=this.$ctx.parent(this.cfg.selector);
this.epgEndpoint=this.$ctx.data(this.cfg.dataEpgEndpoint);
this.epgScript=this.$ctx.data(this.cfg.dataEpgScript);
this.currency=this.$ctx.data(this.cfg.dataCurrency);
this.amount=this.$ctx.data(this.cfg.dataAmount);
this.owningEntity=this.$ctx.data(this.cfg.dataOwningEntity);
this.widgetDebug=this.$ctx.data(this.cfg.dataWidgetDebug);
this.testMerchantId=this.$ctx.data(this.cfg.dataTestMerchantId);
this.prefixMerchantId=this.$ctx.data(this.cfg.dataPrefixMerchantId);
this.technicalErrorMessage=this.$ctx.data(this.cfg.dataTechnicalErrorMessage);
this.paymentErrorMessage=this.$ctx.data(this.cfg.dataPaymentErrorMessage);
this.nameOnCardLabel=this.$ctx.data(this.cfg.dataNameOnCardLabel);
this.creditCardLabel=this.$ctx.data(this.cfg.dataCreditCardLabel);
this.expiryMonthLabel=this.$ctx.data(this.cfg.dataExpiryMonthLabel);
this.expiryYearLabel=this.$ctx.data(this.cfg.dataExpiryYearLabel);
this.securityCodeLabel=this.$ctx.data(this.cfg.dataSecurityCodeLabel);
this.creditCardPlaceholder=this.$ctx.data(this.cfg.dataCreditCardPlaceholder);
this.expiryMonthPlaceholder=this.$ctx.data(this.cfg.dataExpiryMonthPlaceholder);
this.expiryYearPlaceholder=this.$ctx.data(this.cfg.dataExpiryYearPlaceholder);
this.invalidErrorBase=this.$ctx.data(this.cfg.dataInvalidErrorBase);
this.requiredErrorBase=this.$ctx.data(this.cfg.dataRequiredErrorBase);
this.$eCommError=a(this.cfg.eCommError);
this.$ecomCheckboxSelector=a(this.cfg.ecomCheckboxSelector);
this.ecomCheckboxSelectorId=gmwp.util.getModuleId(this.$ecomCheckboxSelector);
this.$requestType=a(this.cfg.requestType);
this.requestTypeId=gmwp.util.getModuleId(this.$requestType);
this.$dealerSelectorV2=a(this.cfg.dealerSelectorV2);
this.dealerSelectorV2Id=gmwp.util.getModuleId(this.$dealerSelectorV2)
},prepare:function(){if(this.ecomCheckboxSelectorId>-1){this.sandbox.subscribe(this.ecomCheckboxSelectorId,this)
}if(this.requestTypeId>-1){this.sandbox.subscribe(this.requestTypeId,this)
}if(this.dealerSelectorV2Id>-1){this.sandbox.subscribe(this.dealerSelectorV2Id,this)
}},checkSelectedDealer:function(){var b=a(".gmit-gf-input-checkbox").filter(":checked").length;
if(b<2){return false
}else{return true
}},onGmitGfEcomDealerSelected:function(b){if(b.bac>-1){this.$ctx.show();
this.merchantId=this.prefixMerchantId+b.bac;
this.initWidget()
}else{this.$ctx.hide()
}},initWidget:function(){var b=this;
if(b.testMerchantId){b.merchantId=b.testMerchantId
}gmwp.util.loadScript(b.epgScript).then(function(){b.widget=new GMPaymentUI({frame:"#widget",debug:b.widgetDebug,merchantId:b.merchantId,clearErrorOnChange:true,clearErrorOnFocus:false,clearErrorOnBlur:false,showCardImages:true,disableJavaScriptClickjackMitigation:true,formFields:{nameOnCard:{label:b.nameOnCardLabel,placeholder:"",requiredError:b.requiredErrorBase+" ("+b.nameOnCardLabel+")",invalidError:b.invalidErrorBase+" "+b.nameOnCardLabel},creditCard:{label:b.creditCardLabel,placeholder:b.creditCardPlaceholder,requiredError:b.requiredErrorBase+" ("+b.creditCardLabel+")",invalidError:b.invalidErrorBase+" "+b.creditCardLabel},expiryMonth:{label:b.expiryMonthLabel,placeholder:b.expiryMonthPlaceholder,requiredError:b.requiredErrorBase+" ("+b.expiryMonthLabel+")",invalidError:b.invalidErrorBase+" "+b.expiryMonthLabel},expiryYear:{label:b.expiryYearLabel,placeholder:b.expiryYearPlaceholder,requiredError:b.requiredErrorBase+" ("+b.expiryYearLabel+")",invalidError:b.invalidErrorBase+" "+b.expiryYearLabel},securityCode:{label:b.securityCodeLabel,placeholder:"",requiredError:b.requiredErrorBase+" ("+b.securityCodeLabel+")",invalidError:b.invalidErrorBase+" "+b.securityCodeLabel}},onSuccessCallback:a.proxy(b.widgetSuccess,b),onErrorCallback:a.proxy(b.showCredentialError,b)})
})
},onValidateEcommCredentials:function(b){this.deferredObj=b.promise;
this.validForm=b.valid;
this.$eCommError.html("");
if(this.checkSelectedDealer()){this.widget.submit()
}else{this.deferredObj.reject()
}},widgetSuccess:function(f){if(this.validForm&&!this.flagSubmitPay){var b,c=this,e=a("#state").val(),h=a("#address_1").val()+" "+a("#address_2").val(),d=a("#gmit-gf-postal-code").val(),g=a("#cityname").val();
a("#gmit-gf-hidden-comments").append(" Session ID: "+f.sessionId);
b={sessionId:f.sessionId,merchantId:this.merchantId,nameOnCard:f.payment.nameOnCard,device:{ipAddress:f.customer.ipAddress},order:{currency:this.currency,amount:this.amount,owningEntity:this.owningEntity},billing:{address:{stateProvince:e,street:h,postcodeZip:d,country:"USA",city:g}}};
a.ajax({type:"POST",url:c.epgEndpoint,headers:{"Content-Type":"application/json"},data:JSON.stringify(b),success:a.proxy(c.submitSuccessToRequestType,c),fail:a.proxy(c.showPaymentError,c),dataType:"json"})
}else{this.deferredObj.resolve()
}},submitSuccessToRequestType:function(d){switch(d.result){case"SUCCESS":this.flagSubmitPay=true;
this.deferredObj.resolve();
break;
case"ERROR":this.$eCommError.html(this.paymentErrorMessage);
this.deferredObj.reject();
break;
case"FAILURE":var c=d.response.gatewayCode,f={APPROVED:3,UNSPECIFIED_FAILURE:2,DECLINED:2,TIMED_OUT:1,EXPIRED_CARD:2,INSUFFICIENT_FUNDS:2,ACQUIRER_SYSTEM_ERROR:2,SYSTEM_ERROR:1,NOT_SUPPORTED:1,DECLINED_DO_NOT_CONTACT:2,ABORTED:3,BLOCKED:2,CANCELLED:3,DEFERRED_TRANSACTION_RECEIVED:3,REFERRED:2,AUTHENTICATION_FAILED:3,INVALID_CSC:2,LOCK_FAILURE:1,SUBMITTED:3,NOT_ENROLLED_3D_SECURE:3,PENDING:3,EXCEEDED_RETRY_LIMIT:1,DUPLICATE_BATCH:1,DECLINED_AVS:2,DECLINED_CSC:2,DECLINED_AVS_CSC:2,DECLINED_PAYMENT_PLAN:2,APPROVED_PENDING_SETTLEMENT:3,PARTIALLY_APPROVED:2,UNKNOWN:2},b={1:this.technicalErrorMessage,2:this.paymentErrorMessage,3:""};
this.$eCommError.html(b[f[c]]);
this.deferredObj.reject();
break;
default:this.$eCommError.html(this.technicalErrorMessage);
this.deferredObj.reject()
}},showCredentialError:function(b){gmwp.util.error("gmit_gf_ecommerce::widgetError",b);
this.$eCommError.html(this.technicalErrorMessage);
this.deferredObj.reject()
},showPaymentError:function(){this.$eCommError.html(this.$invalidErrorBase);
this.deferredObj.reject()
}})
})(gmwp.$,gmwp._);
...
(function(b,a){b.extend(true,gmwp.conf.cmp,{gmit_gf_map:{name:"gmit_gf_map",dataApiKey:"api-key",dataGoogleApiUrl:"google-api-url",initMapFunction:"window.gmitGfInitMap",latDrilldown:"coordinates.latitude",lngDrilldown:"coordinates.longitude",selector:".gmit-gf-map",listSelector:".gmit-gf-list",markerImgSrc:"https://web.archive.org/web/20180622052035/http://www.google.com/mapfiles/marker{label}.png"}});
...
}},submitEcommValidation:function(d){var c=b.Deferred();
this.fire("validateEcommCredentials",{valid:d,promise:c},[this.id]);
return c.promise()
...
}if(b(this.cfg.eCommerceCheckbox).filter(":checked").length){this.submitEcommValidation(false)
...
}},submitForm:function(h){var f=this,d=b(h.currentTarget),c=true,g=this.setTestDealerBac&&this.testDealerBac?a.split(this.testDealerBac,","):a.split(d.find(this.cfg.dealerIdClass).filter(":not("+this.cfg.chosenDealerIdClass+")").val(),",");
...
if(c){a.each(this.$questions,b.proxy(function(j){var i=b('<input type="hidden" name="salesLead.qIds">'),e=b(j),m=e.data("questions")?e.data("questions"):e.data("callers"),k=m&&m.length===1?m[0].key:"",l=m&&m.length===1?m[0].value:"";
if(k){if(e.is(":radio")||e.is(":checkbox")){if(e.is(":checked")){i.val(k+"="+l)
}}else{if(e.is("select")){if(e.find("option:selected").filter("[value]").length>0){i.val(k+"="+e.find("option:selected").val())
}}else{if(e.val()){i.val(k+"="+e.val())
}}}if(e.attr("name").indexOf("salesLead.questionCodes")===0){if(i.val()){d.append(i)
}}else{if(this.commentFieldCheck&&e.length){this.fire("gmitGfAddValuesToCommentField",e,[this.commentFieldId])
}}}},this));
if(b(this.cfg.eCommerceCheckbox).filter(":checked").length){this.submitEcommValidation(true).then(function(){b.ajax({url:f.actionUrl,data:d.serialize(),processData:false,method:"POST",statusCode:{404:b.proxy(f.responseFail,f)},success:b.proxy(f.responseSuccess,f),fail:b.proxy(f.responseFail,f)})
},function(){return false
})
}else{if(!g){b.ajax({url:this.actionUrl,data:d.serialize(),processData:false,method:"POST",statusCode:{404:b.proxy(this.responseFail,this)},success:b.proxy(this.responseSuccess,this),fail:b.proxy(this.responseFail,this)})
}else{a.each(g,b.proxy(function(e){d.find(this.cfg.dealerIdClass).filter(this.cfg.chosenDealerIdClass).val(e);
b.ajax({url:this.actionUrl,data:d.serialize(),processData:false,method:"POST",statusCode:{404:b.proxy(this.responseFail,this)},success:b.proxy(this.responseSuccess,this),fail:b.proxy(this.responseFail,this)})
},this))
...
this.$criteriaError=c.find(this.cfg.criteriaErrorSelector);
...
},onGmitGfAddItem:function(e){var c=this.$selectItemTmpl.clone().removeClass(this.cfg.hideClass).removeClass(this.cfg.selectItemTmplClass).addClass(this.cfg.selectItemClass),d,f;
...
if(e.searchResultData){c.attr("data-item",e.searchResultData)
}if(e&&a.hasIn(e,"searchResultRows")){a.each(e.searchResultRows,function(g){d="{"+g.resultType+"}";
f=a.replace(c.html(),d,g.resultText);
c.html(f)
})
}if(/{[^{}]*}/mg.test(c.html())){f=a.replace(c.html(),/{[^{}]*}/,"");
c.html(f)
...
},onGmitGfShowCriteriaError:function(c){this.onGmitGfToggleDisplaySearchList(false);
this.$ctx.removeClass(this.cfg.hideClass);
this.$criteriaError.html(c)
...
}},onGmitGfSelectFirstItem:function(){this.$resultItems.find(this.cfg.selectItem).find("input").click().prop("checked",true)
},onGmitGfClearSearchResult:function(){this.$searchError.html("");
this.$searchHeadline.html("");
this.onGmitGfClearSearchList()
},onGmitGfClearCriteriaError:function(){this.$criteriaError.html("")
...
}},onChangeVehicle:function(){if(this.thankYouId>0){this.fire("gmitResetThankYouParagraph",[this.thankYouId])
}if(this.requestTypeId>0){this.fire("gmitGfChangeVehicle",[this.requestTypeId])
}if(this.followOnId>0){this.fire("resetFollow",[this.followOnId])
...
(function(b,a){b.extend(true,gmwp.conf.cmp,{gmit_gf_vehicle_selector_v2:{name:"gmit_gf_vehicle_selector_v2",selector:".gmit-gf-vehicle-selector-v2",caller:"x-caller",carlineDetailWrapper:".gmit-gf-carline-detail-wrapper",co2Selector:".gmit-gf-co2-data",dataDropdownPreselectDrilldowns:"dropdown-preselect-drilldowns",firstDropdownType:"first-dropdown-type",dataEndPoints:"end-point-list",dataHiddenFieldNames:"hidden-field-names",dataMarketXml:"market-xml",dataVdsParams:"vds-params",dropdownListSelector:".gmit-gf-dropdown-list",filter:"x-filters",imageSelector:".gmit-gf-carline-obj",linkSelector:".gmit-gf-link",pricingSelector:".gmit-gf-pricing",textGroupSelector:".gmit-gf-text-group"}});
...
this.firstDropdownType=c.data(this.cfg.firstDropdownType);
...
a.pullAt(this.preSelectValues,i[0].dropdownExcludedIndices);
...
if(!g||f){if(this.dropdownListCheck){e.index=-1;
this.fire("gmitGfResetDropdowns",e,[this.dropdownListId])
...
}a.each(h,b.proxy(function(m){e.option=this.regionConfig.setupOptionList(m,this.firstDropdownType);
...
/*
     FILE ARCHIVED ON 05:20:35 Jun 22, 2018 AND RETRIEVED FROM THE
     INTERNET ARCHIVE ON 07:05:31 May 01, 2023.
...
/*
playback timings (ms):
  captures_list: 114.066
  exclusion.robots: 0.065
  exclusion.robots.policy: 0.057
  RedisCDXSource: 0.602
  esindex: 0.006
  LoadShardBlock: 96.417 (3)
  PetaboxLoader3.datanode: 108.617 (5)
  load_resource: 1434.673 (2)
  PetaboxLoader3.resolve: 1353.34 (2)
*/