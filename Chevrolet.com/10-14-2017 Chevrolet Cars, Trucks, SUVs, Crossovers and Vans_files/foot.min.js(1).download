...
/*!
 * @preserve
 * @date      Mon Sep 25 2017 20:36:44 GMT-0700 (MST)
 * @time      1506397011407
 * @version   1.2.1
...
return{src:d}
}}],hiddenFields:{configurationUrl:{fire:"gmitGfUpdateHiddenField",drilldowns:[],getParams:function(c){var d=c?c:"";
return{text:d,index:0}
}},segment:{fire:"gmitGfUpdateHiddenField",drilldowns:[],getParams:function(c){var d=c?c:"";
return{text:d,index:1}
}},make:{fire:"gmitGfUpdateHiddenField",drilldowns:[],getParams:function(c){var d=c?c:"";
return{text:d,index:2}
}},RPO:{fire:"gmitGfUpdateHiddenField",drilldowns:["RPO"],getParams:function(c){var d=c?c:"";
return{text:d,index:3}
}}},queryParameters:[{callerType:"byo",dropdownHide:true,setPrice:true,setImage:true,dropdownPreselectDrilldowns:["x-modelyear","x-carline","x-bodystyle","trimVcCode"],ajaxCall:{type:"GET",dataType:"xml",url:""},hiddenFieldNames:["RPO"]},{callerType:"ipe",dropdownHide:true,setPrice:true,setImage:true,dropdownPreselectDrilldowns:["x-modelyear","x-carline","x-bodystyle","trimVcCode"],ajaxCall:{type:"GET",dataType:"xml",url:""},hiddenFieldNames:[]},{callerType:"btl",dropdownHide:true,setPrice:true,setImage:true,dropdownPreselectDrilldowns:["x-modelyear","x-carline","x-bodystyle","trimVcCode"],ajaxCall:{type:"GET",dataType:"xml",url:""},hiddenFieldNames:[]},{callerType:"vco",dropdownHide:false,setPrice:false,setImage:false,dropdownPreselectDrilldowns:["x-modelyear","x-bodystyle"],hiddenFieldNames:[]},{callerType:"fs",dropdownHide:true,setPrice:true,setImage:true,dropdownPreselectDrilldowns:["x-modelyear","x-carline","x-bodystyle","trimVcCode"],ajaxCall:{type:"GET",dataType:"xml",url:""},hiddenFieldNames:[]},{callerType:"mbtl",dropdownHide:false,setPrice:true,setImage:true,ajaxCall:{type:"GET",dataType:"xml",url:""},hiddenFieldNames:[]},{callerType:"mvco",dropdownHide:false,setPrice:true,setImage:true,ajaxCall:{type:"GET",dataType:"xml",url:""},hiddenFieldNames:[]}],dropdownPreselectDrilldowns:["x-modelyear","x-carline","x-bodystyle","trimVcCode"],dropdownDataDrilldown:"options",getFirstDropdownData:function(c){var d=[];
...
},loadQueryString:function(c){var g=decodeURI(document.location.search.replace("?","").replace(/\+/g,"%20"));
if(g){g=g.split("&");
for(var d=0;
d<g.length;
...
a.each(d,function(g){var e="";
if(b.cookie(g)){e=b.cookie(g);
if(g==="cookie_customer_location"&&e){c=JSON.parse(decodeURIComponent(e));
...
}}else{if(g==="GMWP_location"&&e){var f='{"'+decodeURI(e).replace(/"/g,'\\"').replace(/,/g,'","').replace(/=/g,'":"')+'"}';
c=JSON.parse(f);
if(a.hasIn(c,"zip")&&c.zip){c.zip=c.zip.split(/[+-]+/);
if(c.zip[0]){a.set(c,"zipcode",c.zip[0])
...
return c
},parseQueryString:function(){var e=document.location.search.replace("?",""),d=e.split("="),c=d.length>1?'{"'+decodeURI(e).replace(/"/g,'\\"').replace(/&/g,'","').replace(/=/g,'":"')+'"}':"{}";
return e?JSON.parse(this.sanitizeData(c)):""
...
},countCharacters:function(i){var h=b(i.currentTarget),g=h.val(),f=gmwp.util.isNumber(h.attr(this.cfg.maxLengthAttr))?h.attr(this.cfg.maxLengthAttr):0,d=g?g.length:0,c=f-d;
if(f>0){if(d>f&&g){h.val(g.substring(0,f))
}this.$counter.html(c)
}return false
...
},onGmitGfAddValuesToCommentField:function(d){var c="",e="";
a.each(d,b.proxy(function(j){var f=[],h="",l="",g=b(j),k=g.data("questions")?g.data("questions"):g.data("callers"),i=k&&k.length===1?k[0].key:"";
if(i){if(g.hasClass("gmit-gf-input-radio")||g.hasClass("gmit-gf-input-checkbox")){if(g.is(":checked")){f=g.next();
h=i||f.text();
l=k&&k.length?k[0].value:g.val()
}}else{if(g.hasClass(".gmit-gf-comment")){c=g.val();
e=i
}else{f=g.prev();
h=i||f.text();
l=g.val()
}}if(l){this.$commentField.append(h+"="+l+" ")
}}},this));
if(c){this.$commentField.append(e+"="+c+" ")
}}})
})(gmwp.$,gmwp._);
...
(function(b,a){b.extend(true,gmwp.conf.cmp,{gmit_gf_map:{name:"gmit_gf_map",dataApiKey:"api-key",dataGoogleApiUrl:"google-api-url",initMapFunction:"window.gmitGfInitMap",latDrilldown:"coordinates.latitude",lngDrilldown:"coordinates.longitude",selector:".gmit-gf-map",listSelector:".gmit-gf-list",markerImgSrc:"https://web.archive.org/web/20171014012005/http://www.google.com/mapfiles/marker{label}.png"}});
...
if(d&&a.hasIn(d,"newText")){c.newText=this.formatPrice(d.newText);
...
}},onGmitGfLinkClick:function(){var c=b();
if(this.$form.length){c=this.$form.find('input[type="radio"]').clone();
if(this.dealerSelectorV2Id>0){this.fire("gmitGfClearDealerList",[this.dealerSelectorV2Id]);
this.fire("resetshowDealerErrorVisible",[this.dealerSelectorV2Id])
}a.each(this.commentsId,b.proxy(function(d){this.fire("gmitGfResetCharacterCounter",[d])
},this));
this.$ctx.find("span .icon-minus").parent().click();
this.$form.children("div").removeClass("error");
this.$form.children("label").removeClass("error");
if(this.testDealerParagraphCheck){this.fire("gmitGfToggleTestDealerParagraph",{toggleFlag:false},[this.testDealerParagraphId])
}this.$form[0].reset();
a.each(c,b.proxy(function(e){var d=b(e);
this.$form.find("#"+d.attr("id")).val(d.prop("defaultValue"))
},this));
this.$form.find('[name="salesLead.qIds"]').remove();
this.fire("gmitGfClearDisabledBrochures",[this.id]);
this.$form.slideDown(this.cfg.animateSpeed);
this.onApplyBusinessRules();
this.$form.find(this.cfg.formElementSelectors).filter(":visible:first").focus()
...
(function(b,a){b.extend(true,gmwp.conf.cmp,{gmit_gf_text_field:{name:"gmit_gf_text_field",autocomplete:".js-gmit-gf-autocomplete",caller:"x-caller",dataCallers:"callers",dataTestDealerValue:"test-dealer-value",populateField:"populate-field",selector:".gmit-gf-input-text"}});
...
},finish:function(){var e=this.$inputElement.data(this.cfg.dataCallers),c=a.get(gmwp.conf.gf.queryObject,this.cfg.caller,""),d=a.filter(e,{key:c},[]);
this.$inputElement.off("focus");
if(this.$inputElement.data(this.cfg.populateField)){this.loadFieldFromCookie(this.$inputElement.data(this.cfg.populateField))
...
if(d){if(a.hasIn(d,"text")&&d.text){c.val(d.text);
Foundation.libs.abide.validate(c,{type:""})
}else{c.val("")
...
(function(b,a){b.extend(true,gmwp.conf.cmp,{gmit_gf_vehicle_selector_v2:{name:"gmit_gf_vehicle_selector_v2",selector:".gmit-gf-vehicle-selector-v2",caller:"x-caller",carlineDetailWrapper:".gmit-gf-carline-detail-wrapper",co2Selector:".gmit-gf-co2-data",dataEndPoints:"end-point-list",dataMarketXml:"market-xml",dataVdsParams:"vds-params",dropdownListSelector:".gmit-gf-dropdown-list",filter:"x-filters",imageSelector:".gmit-gf-carline-obj",linkSelector:".gmit-gf-link",pricingSelector:".gmit-gf-pricing",textGroupSelector:".gmit-gf-text-group"}});
...
this.sandbox.subscribe(this.id,this)
},prepare:function(){this.xFilters=a.hasIn(this.queryString,this.cfg.filter)?this.queryString[this.cfg.filter]:"";
if(this.imageCheck){this.sandbox.subscribe(this.imageId,this)
}if(this.co2Check){this.sandbox.subscribe(this.co2Id,this)
}if(this.pricingCheck){this.sandbox.subscribe(this.pricingId,this)
}if(this.dropdownListCheck){this.sandbox.subscribe(this.dropdownListId,this)
}if(this.textGroupCheck){this.sandbox.subscribe(this.textGroupId,this)
}if(this.linkCheck){this.sandbox.subscribe(this.linkId,this)
}},finish:function(){var d=this.regionConfig.queryParameters,h=[],g="",c="",e="",f="";
if(this.linkCheck){this.fire("gmitGfHideLink",[this.linkId])
}if(this.queryString&&a.hasIn(this.queryString,this.cfg.caller)){a.each(d,b.proxy(function(i){if(this.queryString[this.cfg.caller]===i.callerType&&i.dropdownHide){h=i.hiddenFieldNames;
if(this.dropdownListCheck){this.fire("gmitGfHideDropdownList",[this.dropdownListId])
}if(this.queryString.agencyrolecode){this.updateHiddenFields(["agencyRoleCode"],this.queryString)
}if(h.length){this.updateHiddenFields(h,this.queryString)
}if(this.queryString["x-ngBackLinkURL"]){b.ajax({type:"GET",dataType:"json",url:decodeURIComponent(this.queryString["x-ngBackLinkURL"])}).done(b.proxy(function(j){if(j){this.updateComponent(j,this.regionConfig.headlines,this.textGroupId);
this.updateComponent(j,this.regionConfig.pricing,this.pricingId);
this.updateComponent(j,this.regionConfig.co2,this.co2Id);
this.updateComponent(j,this.regionConfig.image,this.imageId);
this.updateHiddenFields(h,j)
}},this));
this.$carlineDetailWrapper.show();
if(this.co2Check){this.fire("gmitGfShowCo2Image",[this.co2Id])
}}else{if(this.linkCheck&&this.marketXml&&this.queryString[this.cfg.caller]==="fs"){this.loadFirstDropdown();
this.fire("gmitGfSetupChangeEvents",[this.dropdownListId]);
this.fire("gmitGfSetupClickEvent",[this.linkId]);
i.ajaxCall.url=this.marketXml;
b.ajax(i.ajaxCall).done(b.proxy(function(j){if(j){g=this.getConfigStringFromXml(j,[this.queryString["x-carline"],this.queryString["x-modelyear"]],[this.queryString["x-bodystyle"]],[this.queryString.trimVcCode]);
c=g.attr("title");
this.queryString.trimDesc=c;
this.updateComponent(this.queryString,this.regionConfig.headlines,this.textGroupId);
this.$carlineDetailWrapper.show();
this.fire("gmitGfShowLink",[this.linkId]);
this.fire("gmitGfUpdateVehicle",this.thankYouVehicle,[this.id])
}},this))
}else{this.updateComponent(this.queryString,this.regionConfig.headlines,this.textGroupId);
this.$carlineDetailWrapper.show();
if(i.setPrice){if(this.queryString.msrpPrice){this.updateComponent(this.queryString,this.regionConfig.pricing,this.pricingId)
}else{if(this.marketXml){i.ajaxCall.url=this.marketXml;
b.ajax(i.ajaxCall).done(b.proxy(function(j){if(j){g=this.getConfigStringFromXml(j,[this.queryString["x-carline"],this.queryString["x-modelyear"]],[this.queryString["x-bodystyle"]],[this.queryString.trimVcCode]);
f=g.attr("formatted_price");
this.updateComponent({msrpPrice:f},this.regionConfig.pricing,this.pricingId);
this.fire("gmitGfUpdateVehicle",this.thankYouVehicle,[this.id])
}},this))
}}}if(i.setImage&&this.marketXml){i.ajaxCall.url=this.marketXml;
b.ajax(i.ajaxCall).done(b.proxy(function(j){if(j){g=this.getConfigStringFromXml(j,[this.queryString["x-carline"],this.queryString["x-modelyear"]],[this.queryString["x-bodystyle"]],[this.queryString.trimVcCode]);
e=g.find("color").attr("imageURL");
this.updateComponent({imageUrl:e},this.regionConfig.image,this.imageId);
this.fire("gmitGfUpdateVehicle",this.thankYouVehicle,[this.id])
}},this))
}if(this.dropdownListCheck){this.fire("gmitGfSetupChangeEvents",[this.dropdownListId])
}}}}else{if(this.queryString[this.cfg.caller]===i.callerType&&!i.dropdownHide){this.loadFirstDropdown();
if(this.dropdownListCheck){this.fire("gmitGfSetupChangeEvents",[this.dropdownListId])
}}}},this));
this.fire("gmitGfRemoveHideClasses",[this.textGroupId]);
this.fire("gmitGfUpdateVehicle",this.thankYouVehicle,[this.id])
}else{this.loadFirstDropdown();
if(this.dropdownListCheck){this.fire("gmitGfSetupChangeEvents",[this.dropdownListId])
}}},loadPreselect:function(h){var d={callerType:gmwp.conf.gf.queryObject["x-caller"]},k=a.filter(this.regionConfig.queryParameters,d,[]),g=k.length?k[0].dropdownPreselectDrilldowns:[],l="",f="",i=[],n="",j="",m="",e="",o=[],c=this.checkData(this.vdsParams,this.queryString);
if(h&&this.queryString){a.each(g,b.proxy(function(p){if(a.hasIn(h,p)){o.push(h[p])
}else{return o
}},this));
if(k.length&&this.queryString&&a.hasIn(k[0],"ajaxCall")){o=c?c.split("/"):o;
n=this.parseTrimPath(o,"year");
j=this.parseTrimPath(o,"model");
e=this.parseTrimPath(o,"trim");
m=this.parseTrimPath(o,"bodyStyle");
k[0].ajaxCall.url=this.marketXml;
b.ajax(k[0].ajaxCall).done(b.proxy(function(p){if(p){l=this.getConfigStringFromXml(p,[j,n],[m],[e]);
f=l.attr("formatted_config")?l.attr("formatted_config")+" "+l.attr("formatted_drive"):l.attr("formatted_drive");
this.preSelectValues.push(n);
this.preSelectValues.push(m);
this.preSelectValues.push(f);
this.preSelectValues.push(e);
this.loadFirstDropdown(true)
}},this))
}else{this.preSelectValues=a.clone(o)
}}return i
...
}},updateHiddenFields:function(i,e){var d=this.regionConfig.hiddenFields,c="",h="",g={},f={};
if(e&&d&&this.dropdownListCheck){a.each(i,b.proxy(function(j){f=d[j];
c=f.drilldowns;
h=this.checkData(c,e);
g=f.getParams(h);
this.fire(f.fire,g,[this.dropdownListId])
},this))
...
}},onGmitGfLinkClick:function(){var c={},d={};
if(this.linkCheck){this.fire("gmitGfHideLink",[this.linkId])
}if(this.textGroupCheck){this.fire("gmitGfHideTextGroup",[this.textGroupId])
...
}if(this.dropdownListCheck){c.index=0;
d.index=0;
d.value="";
this.fire("gmitGfResetDropdowns",c,[this.dropdownListId]);
this.fire("gmitGfSelectDropdownOption",d,[this.dropdownListId]);
this.fire("gmitGfShowDropdownList",[this.dropdownListId])
...
}if(a.hasIn(k,"hiddenFieldNames")){this.updateHiddenFields(k.hiddenFieldNames,d)
...
}if(this.pricingCheck&&d.setPricing){this.updateComponent(B,v,this.pricingId);
...
}else{if(y.endPointTag==="price"){this.updateComponent(B,v,this.pricingId)
...
/*
     FILE ARCHIVED ON 01:20:05 Oct 14, 2017 AND RETRIEVED FROM THE
     INTERNET ARCHIVE ON 06:41:13 May 01, 2023.
...
/*
playback timings (ms):
  captures_list: 91.413
  exclusion.robots: 0.082
  exclusion.robots.policy: 0.075
  cdx.remote: 0.07
  esindex: 0.008
  LoadShardBlock: 65.709 (3)
  PetaboxLoader3.datanode: 107.132 (5)
  load_resource: 2004.566 (2)
  PetaboxLoader3.resolve: 1837.332 (2)
*/