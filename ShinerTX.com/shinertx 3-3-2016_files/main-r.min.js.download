...
function PackagesUtil(packagesStructure, query) {
    ...
    this.getVersionsByQuery = function(versionsObject) {
        ...
        function getVersionString(value) {
            if (/^\d+$/.test(value)) {
                return 'https://web.archive.org/web/20160303191101/http://localhost' + value;
            }
            if (/^[\d\.]+$/.test(value)) {
                return value;
            }
...
function instrument(serviceTopology, wixBiSession, queryUtil, siteModel) {
    ...
    var base = (serviceTopology.biServerUrl || 'https://web.archive.org/web/20160303191101/http://frog.wix.com').replace(/\/$/, '');
...
function getViewerRjsConfig (serviceTopology) {
    ...
    return {
        ...
        paths: {
            ...
            react: {min: serviceURL('react/0.14.3/react-with-addons.min'), source: '//web.archive.org/web/20160303191101/http://cdnjs.cloudflare.com/ajax/libs/react/0.14.3/react-with-addons'},
            reactDOM: {min: serviceURL('react/0.14.3/react-dom.min'), source: '//web.archive.org/web/20160303191101/http://cdnjs.cloudflare.com/ajax/libs/react/0.14.3/react-dom'},
            reactDOMServer: {min: serviceURL('react/0.14.3/react-dom-server.min'), source: '//web.archive.org/web/20160303191101/http://cdnjs.cloudflare.com/ajax/libs/react/0.14.3/react-dom-server'},
            ...
            io: 'https://web.archive.org/web/20160303191101/https://cdnjs.cloudflare.com/ajax/libs/socket.io/1.4.0/socket.io.min',
...
function render(isServerSide, isPreview, contentCache, queryUtil, siteModel, wixBiSession) {
    ...
    if (queryUtil.getParameterByName('ReactSource') === 'https://web.archive.org/web/20160303191101/http://localhost') {
...
/*global packagesUtil, instrument, queryUtil, overrideScriptsLocationMapFromQuery, prefetchPages, convertSiteModel, fixViewport, getFullRjsConfig, getViewerRjsConfig, getSubdomain, render, convertRendererModel, joinURL, addExperimentsFromQuery*/
    ...
    serviceTopology.staticServerFallbackUrl = serviceTopology.staticServerFallbackUrl || 'https://web.archive.org/web/20160303191101/https://fallback.wix.com/';
...
/*
     FILE ARCHIVED ON 19:11:01 Mar 03, 2016 AND RETRIEVED FROM THE
     INTERNET ARCHIVE ON 04:19:25 Jul 19, 2023.
...
playback timings (ms):
  ...
  load_resource: 59.439
  PetaboxLoader3.resolve: 31.45
*/